...
def train():
    ...
    for epoch in range(config.num_epochs):
        ...
        for batch_ids,batch_mask,batch_segment,batch_label in batch_train:
            tf.logging.info('Epoch:%d,training....'%(epoch + 1))
            ...
            if global_step - last_improved > config.require_improvement:
                tf.logging.info("No optimization over 50 steps, stop training")
                flag = True
                break
        if flag:
            break
...
def test():
    ...
    save_dir = os.path.join(config.output_dir, "checkpoints/textcnn")
    # save_dir = './result/checkpoints/textcnn'
    save_path = os.path.join(save_dir, 'best_validation')
    # save_path = './result/checkpoints/textcnn/best_validation'
    ...
    print(len(set(y_test_cls)),len(set(y_pred_cls)))
